...
class ArgumentsSetter:
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            description='Created collections in the database {0}'.format(MONGO_DATABASES['db_data']),
        )
...
async def create_collections(index_flag: bool, drop_flag: bool) -> bool:
    ...
    mdb.cl = AsyncIOMotorClient(MONGO_CONNECT_STRING)
    mdb.init_db(MONGO_DATABASES['db_data'])
    ...
    if not index_flag:
        json_schema_full_path = Path(BASE_DIR, CREATE_COLLECTIONS_COMMANDS_JSON_FILE)
        await perform_command_from_json_file(mdb, json_schema_full_path)

    json_schema_full_path = Path(BASE_DIR, CREATE_COLLECTIONS_INDEXES_COMMANDS_JSON_FILE)
